...
/
CREATE OR REPLACE EDITIONABLE TRIGGER  "ATAF_BI_TEST_SPEC" 
  before insert or update on "ATAF_TEST_SPEC"               
  for each row  
begin  
  IF inserting THEN
 ...
 END IF;
   IF :NEW.CHANGE_FLAG = 'N' THEN
     :NEW.CHANGE_FLAG := null;
   END IF;
end;
/
...
/
CREATE OR REPLACE EDITIONABLE TRIGGER  "ATAF_TEST_CASE_AIUD" 
AFTER
insert or update or delete on "ATAF_TEST_CASE"
for each row
